...
class WebExplorer:
    def __init__(self):
        ...
        # Static page tracking
        self.static_pages: Set[str] = set()
        self.static_page_descendants: Dict[str, Set[str]] = {}
        
    def _setup_results_dir(self):
        """Set up the results directory, clearing any previous results"""
        # If directory exists, remove it completely
        if os.path.exists(RESULTS_DIR):
            if DEBUG:
                print(f"Wiping exploration results directory: {RESULTS_DIR}")
        ...
        if DEBUG:
            print(f"Created fresh exploration results directory: {RESULTS_DIR}")
    ...
    async def create_agent(self, url: str, parent_id: Optional[str] = None, depth: int = 0) -> Optional[str]:
        ...
        # Check if this URL is a static page descendant and already at max depth for static
        is_static_descendant = False
        static_parent = None
        
        if parent_id and parent_id in self.agents:
            parent_url = self.agents[parent_id].current_url
            # Check if parent is a static page
            if parent_url in self.static_pages:
                is_static_descendant = True
                static_parent = parent_url
            # Check if parent is already a descendant of a static page
            else:
                for static_url, descendants in self.static_page_descendants.items():
                    if parent_url in descendants:
                        is_static_descendant = True
                        static_parent = static_url
                        break
        
        # If this is a static page descendant at depth 1 or more, don't create the agent
        if is_static_descendant and depth > 1:
            if DEBUG:
                print(f"Not creating agent for {url}: already too deep from static page {static_parent}")
            return None
    ...
    async def explore_page(self, agent_id: str):
        ...
        try:
            # Check if this URL is a descendant of a static page
            parent_static = False
            static_ancestry_depth = 0
            
            # If parent exists, check if it's static or descended from static
            if agent.parent_id and agent.parent_id in self.agents:
                parent_url = self.agents[agent.parent_id].current_url
                
                # Check if parent is a static page
                if parent_url in self.static_pages:
                    parent_static = True
                    static_ancestry_depth = 1
                    
                    # Add this URL as a descendant of the static page
                    if parent_url not in self.static_page_descendants:
                        self.static_page_descendants[parent_url] = set()
                    self.static_page_descendants[parent_url].add(agent.current_url)
                    
                # Check if parent is a descendant of a static page
                else:
                    for static_url, descendants in self.static_page_descendants.items():
                        if parent_url in descendants:
                            parent_static = True
                            static_ancestry_depth = 2  # At least 2 levels deep from static
                            break
            
            # If this page is already too deep from a static page, close agent
            if parent_static and static_ancestry_depth > 1:
                if DEBUG:
                    print(f"URL {agent.current_url} is too deep from a static page. Closing agent {agent_id[:8]}")
            ...
            is_static = await self.is_static_page(agent.page, agent.current_url, page_title, html_content)
            
            # Mark as static for future reference
            if is_static:
                self.static_pages.add(agent.current_url)
                self.static_page_descendants[agent.current_url] = set()
                
                if DEBUG:
                    print(f"Marked as static page: {agent.current_url}")
            ...
            prompt = f"""
            You are a web application testing agent. Your goal is to systematically test the core features and main user flows of this web application.

            Current page: {agent.current_url}
            Page title: {page_title}
            Current exploration depth: {agent.depth}
            Page type: {"STATIC" if agent.current_url in self.static_pages else "DYNAMIC"}
            ...
            1. Which links are most likely to lead to core features or main user flows? (Choose up to 3)
            2. Should I test multiple paths or focus on a single path?
            3. What is the main functionality to test on this page?

            Respond in the following JSON format:
            {{
                "reasoning": "Why you chose these links and what core features they represent",
                "selected_links": [index numbers of links to test, e.g. [1, 3, 5]],
                "branch_strategy": "single" or "multiple",
                "test_focus": "What core feature or user flow to test on this page"
            }}
            ...
            self.exploration_log.append({
                "agent_id": agent_id,
                "url": agent.current_url,
                "is_static": agent.current_url in self.static_pages,
                "decision": {
            ...
            # For static pages, modify branching strategy: no branching and limit descendants
            is_static_page = agent.current_url in self.static_pages
            is_static_descendant = any(agent.current_url in descendants for descendants in self.static_page_descendants.values())
            
            if is_static_page or is_static_descendant:
                # For static pages or their descendants, only explore a single path
                branch_strategy = "single"
                
                # Limit to at most 1 link
                if selected_indices:
                    selected_indices = [selected_indices[0]]
                
                if DEBUG:
                    print(f"Static page or descendant: limiting to single path exploration")
                    
                # Handle static pages - if already at depth >=1 from static, don't go deeper
                if is_static_descendant and agent.depth >= 1:
                    if DEBUG:
                        print(f"Already at or beyond depth 1 from a static page, closing agent {agent_id[:8]}")
                    await self.close_agent(agent_id)
                    return
            ...
            for i, element in enumerate(elements[:5]):  # Limit to first 5 elements
                try:
                    ...
                    if visible:
                        ...
                        # Use safe click method from InteractionHandler
    ...
    async def run(self, start_url: str):
        ...
        # Show browser visibility mode
        print(f"Browser mode: {'HEADLESS (hidden)' if BROWSER_HEADLESS else 'VISIBLE (you will see the browsers)'}")
        ...
        while not self.exploration_queue.empty() or self.active_agents_count > 0:
            try:
                ...
                print(f"Static pages: {len(self.static_pages)}, Static descendants: {sum(len(d) for d in self.static_page_descendants.values())}")
        ...
        dynamic_count = len(self.all_visited_urls) - static_count
        print(f"Exploration complete. Visited {len(self.all_visited_urls)} unique URLs in {duration:.2f} seconds.")
        print(f"Pages by type: {dynamic_count} dynamic, {static_count} static")
        print(f"Static page descendants: {sum(len(d) for d in self.static_page_descendants.values())}")
    ...
    def save_results(self, start_url: str, duration: float):
        ...
        results = {
            ...
            "static_pages": list(self.static_pages),
    ...
    async def is_static_page(self, page: Page, url: str, page_title: str, html_content: str) -> bool:
        """
        Enhanced static page detection using network activity and JS files.
        """
        url_lower = url.lower()
        # Existing static patterns...
        static_url_patterns = [
            '/privacy', '/policy', '/terms', '/tos', '/about', 
            '/contact', '/faq', '/help', '/support', '/legal',
            '/cookie', '/disclaimer', '/accessibility', '/sitemap',
            '/copyright', '/license', '/careers', '/jobs',
            '/press', '/news', '/blog/', '/article/', '/story/',
            '/static/', '/assets/', '/media/', '/docs/'
        ]
        for pattern in static_url_patterns:
            if pattern in url_lower:
                return True

        # Check for loaded JS files
        js_files = await page.evaluate("""
            () => Array.from(document.scripts).map(s => s.src).filter(src => src.endsWith('.js'))
        """)
        if len(js_files) < 2:  # Arbitrary threshold; adjust as needed
            # Check for XHR/fetch activity
            has_xhr = await page.evaluate("""
                () => !!window.performance.getEntriesByType('resource').find(e => e.initiatorType === 'xmlhttprequest' || e.initiatorType === 'fetch')
            """)
            if not has_xhr:
                return True
        ...
        static_title_patterns = [
            'privacy', 'policy', 'terms', 'condition', 'about us', 
            'contact', 'faq', 'help', 'support', 'legal',
            'cookie', 'disclaimer', 'accessibility', 'sitemap',
            'copyright', 'license', 'career', 'job',
            'press', 'news', 'blog', 'article'
        ]
        
        for pattern in static_title_patterns:
            if pattern in title_lower:
                if DEBUG:
                    print(f"Classified as static page due to title pattern '{pattern}': {url}")
                return True
        
        # Check for lack of interactive elements (simplified check)
        interactive_count = 0
        interactive_patterns = [
            '<button', '<input', '<select', '<textarea', 
            'onclick=', 'onchange=', 'role="button"', 'role="tab"',
            'contenteditable', 'draggable', '<form', 'aria-live'
        ]
        
        for pattern in interactive_patterns:
            interactive_count += html_content.lower().count(pattern)
        
        # If very few interactive elements and long content, likely static
        content_length = len(html_content)
        if content_length > 10000 and interactive_count < 10:
            if DEBUG:
                print(f"Classified as static page due to content pattern: {url}")
            return True
